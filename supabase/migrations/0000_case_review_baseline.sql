-- Case-review baseline schema (banks, clients, cases, …), recovered from git
-- history: commit fa5b786 overwrote 0001_init.sql with the tally-module schema,
-- which orphaned migrations 0002–0016 on fresh databases. Numbered 0000 so it
-- sorts before both. Idempotent (if-not-exists / drop-policy-if-exists) — safe
-- to apply to a database that already has these tables.
